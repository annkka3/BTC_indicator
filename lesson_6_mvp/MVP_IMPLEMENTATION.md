...
Реализовать рабочий аналитический сервис, который:
- принимает рыночные данные BTC через TradingView,
- сохраняет их в SQLite,
- формирует **ML‑прогнозы курса Bitcoin с помощью CatBoost**,
- предоставляет результаты через REST API и Telegram‑бот.
...
## ML‑часть (BTC Forecast)

**Алгоритм**
- `CatBoostRegressor`

**Target**
- Цена закрытия BTC или её производная (log‑return)
...
**Features**
- OHLC данные BTC
- Производные фичи (returns, momentum, volatility)
- Рыночный контекст (доминирование и капитализация — опционально)

**Ограничения**
- Модель не выполняет автоторговлю
- Используется только для аналитических сигналов
...
## Синхронизация с архитектурой

- ML‑модель является частью **Application/ML layer**
- Domain не зависит от ML‑реализации
- Infrastructure отвечает за загрузку модели и доступ к данным

---

MVP демонстрирует полный цикл:
данные → признаки → CatBoost‑прогноз → API / Telegram.

Проект готов к дальнейшему расширению,
не нарушая текущих архитектурных границ.
